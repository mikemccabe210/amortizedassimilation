...
from amortized_assimilation.models import MultiObs_EnAF
from amortized_assimilation.operators import filter_obs, mystery_operator


def train(epoch, loader, noise, m, model, optimizer, scheduler, obs_dict, indices, device, hidden_size):
    """ Training loop """
    ntypes = len(obs_dict)
    for batch_y0 in loader:
        ...
        # Init components
        memory = torch.zeros(pred_y1.shape[0] * m , hidden_size, device = device)
...
def test(epoch, start_time, base_data, noise, m, model, obs_dict, indices, device, hidden_size):
    """Test loop"""
    with torch.no_grad():
        state = base_data[0] 
        ...
        pred_y_test, _, _, ens = assimilate_unseen_obs_ens(model, noisy_test, state, m,
                                                           obs_dict, indices, device, hidden_size)
...
def assimilate_unseen_obs_ens(model, data, state, m, obs_dict, indices, device, hidden_size):
    ...
    filtered_obs = []
    ensembles = []
    memory = torch.zeros(m, hidden_size, device = device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_steps', type=int, default=80)
    ...
    parser.add_argument('--do', type=float, default = .1)
    ...
    true_y, true_y_valid, true_y_test = gen_data('lorenz96', t, args.steps_test, args.steps_valid)
    
    # 
    input_types, obs_dict, indices, known_h = obs_configs.lorenz_configs[args.obs_conf]
    ntypes = len(obs_dict)
#     Set up model
    model = MultiObs_EnAF(args.n, args.hidden_size, input_types=input_types, m = args.m, do = args.do)
    ...
    for itr in range(1, args.epochs + 1):
        ...
        start_time = time.time()
        train(itr, loader, args.noise, args.m, model, optimizer, dummy_sched, obs_dict, indices, device, args.hidden_size)
        test(itr, start_time, true_y_valid, args.noise, args.m, model, obs_dict, indices, device, args.hidden_size)

    # Test
    print('---test set results---')
    test(itr, start_time, true_y_test, args.noise, args.m, model, obs_dict, indices, device, args.hidden_size)