...
from amortized_assimilation.data_utils import ChunkedTimeseries, VL20, TimeStack, gen_data
...
def train(epoch, loader, noise, m, model, optimizer, scheduler, obs_dict, indices, device, missing = False):
    ...
    for batch_y0 in loader:
        ...
        ensembles = []
        # Init components
        memory = torch.zeros(pred_y1.shape[0] * m, 6, 36, device=device)
...
def test(epoch, start_time, base_data, noise, m, model, obs_dict, indices, device, missing = False):
    ...
    with torch.no_grad():
        state = base_data[0] 
        state = state.unsqueeze(1).repeat(1, m, 1, 1)
...
def assimilate_unseen_obs_ens(model, noise, data, state, m, obs_dict, indices, device, missing):
    ...
    filtered_obs = []
    ensembles = []
    memory = torch.zeros(m, 6, 36, device = device)
    ...
    for i, obsi in enumerate(data):
        ...
        if missing:
            ...
            obs = obsi.unsqueeze(1).repeat(1, m, 1, 1)
            mask = None
            obs_type = '0'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dynamics', type=str, default='vl20')
    ...
    t = torch.arange\
        (0, args.train_steps*args.step_size, args.step_size)
    true_y, true_y_valid, true_y_test = gen_data('vl20', t, args.steps_test,
    ...
    input_types, obs_dict, indices, known_h = obs_configs.vl_configs[args.obs_conf]
    input_types, _, _, known_h = obs_configs.vl_configs['full_obs']
    ...
    model = MultiObs_ConvEnAF(args.n, args.hidden_size, input_types=input_types,
                             m = args.m, missing = missing, do = args.do, int_kwargs=int_kwargs,
                              interval=interval, in_channels=2, ode_func=VL20())
    # Get param count
