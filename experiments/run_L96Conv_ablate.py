    ...
    for batch_y0 in loader:
        ...
        for i, xi in enumerate(batch_y0):
            ...
            if missing:
                ...
                x[:, :, indices[str(i_type % ntypes)]] = (obs_dict[str(i_type % ntypes)](xi)).unsqueeze(1).repeat(1, m,
                                                                                                                  1)
...
def assimilate_unseen_obs_ens(model, noise, data, state, m, obs_dict, indices, device, missing):
    ...
    for i, obsi in enumerate(data):
        ...
        if missing:
            ...
            obs[:, :, indices[str(i % len(obs_dict))]] = (obs_dict[str(i % len(obs_dict))](obsi)).unsqueeze(1).repeat(1,
                                                                                                                      m,
                                                                                                                      1)
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.epochs + 1):
        ...
        if itr % args.checkpoint == 0:
            torch.save(model.state_dict(), folder_name + '/convref_%s_%s_%.4f_%.1fstd_%diters_%dfilt_ablate'
