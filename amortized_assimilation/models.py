...
from .model_parts import UNet, SpatialDropout1d
...
class UEnAF(nn.Module):
    ...
    def __init__(self, obs_size, hidden_size, state_size, ode_func, m, do = .1, missing = False):
        ...
        if missing:
            ...
            n_in = 10
        self.base_net = UNet(state_size, n_in, 7, 1, hidden_size)
    ...
    def forward(self, observation, state, memory, mask = None, tols = (1e-3, 1e-5),
        ...
        x, memory = base.split([1, 6], 1)
        # Sigmoid subnets
        # Autoregressive state estimate updates
        # Forward model
        # print('after net', x.shape, memory.shape)
        # TODO: This should definitely happen at the beginning of an assimilation
        # cycle instead of the end but went here in an earlier version where it 
        # made more sense. 
        state = odeint(self.ode_func, x, interval, rtol=rtol, atol=atol,
        ...
        # Debugging tool to track forward evals for adaptive integrators
        self.ode_func.fe = 0
        # print('after int', x.shape, memory.shape)
...
class MultiObs_UEnAF(nn.Module):
    ...
    def __init__(self, state_size, hidden_size, input_types = {},
        ...
        super(MultiObs_UEnAF, self).__init__()
        ...
        if len(input_types) != 0:
            ...
            for k, v in input_types.items():
                label = k
                subnet = UEnAF(v, hidden_size, state_size, ode_func, m = m, missing = missing, do = do)
                in_dict[label] = subnet
            self.input_mods = nn.ModuleDict(in_dict)
        # This path is almost certainly broken by now
        else:
...
class KSConvEnAF(nn.Module):
    ...
    def __init__(self, obs_size, hidden_size, state_size, ode_func, m, do = .1, device = None):
        ...
        n_in = 7
        # Linear components
        self.scale_up = nn.Sequential(nn.Conv1d(n_in, hidden_size, kernel_size = 5,
                                         padding = 2, padding_mode = 'circular',
                                         groups = 1),
                                      nn.LayerNorm([hidden_size, 128], elementwise_affine=False),
                                                   nn.LeakyReLU()
                                                   )

        self.lin_net1 = nn.Sequential(nn.Conv1d(hidden_size, hidden_size, kernel_size = 5,
                                         padding = 2, padding_mode = 'circular',
                                         groups = 1),
                                     nn.LayerNorm([hidden_size, 128], elementwise_affine=False),
                                    nn.LeakyReLU(),
        ...
        self.lin_net2 = nn.Sequential(
                                 ...
                                 nn.LeakyReLU(),
                                     SpatialDropout1d(do),
                                nn.Conv1d(hidden_size, 10, kernel_size = 5,
                                         padding = 2, dilation = 1, padding_mode = 'circular',
                                         groups = 1),
                                   )
